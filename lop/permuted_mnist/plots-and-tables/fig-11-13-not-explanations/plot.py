...
for (METRIC, TITLE, YLABEL, MULTS, FIGN, SCILIMITS, MULT) in [
    ('weight_magnitude/layer_', "Layerwise Distribution of Weight Magnitudes", "Weight Magnitude", None, "weight_magnitude", (4, 4), 1),
    ('probing_accuracy_pre/layer_', "Feature Usefulness per Layer", "Probing Accuracy (%)", None, "probing_accuracy", None, 100),
    ('effective_rank/layer_', "Effective Ranks of Layerwise Feature Maps", "Effective Rank", None, "effective_rank", None, 1),
    ('dead_neurons/layer_', "Layerwise Distribution of Dead Neurons", "Dead Neuron Count", None, "dead_neurons", None, 1),
    ('avg_scores/layer_', "Layerwise Utility Score Distribution", "Average Utility Score", None, "avg_scores", None, 1),
    ('hessian/layer_', "Loss Space Curvature per Layer", "Rank of Hessian", None, "hessian", (3, 3), 1),
]:
    STEP_METRIC = 'task'
    MAX_STEP = 400
    BIN_SIZE = 10
    ...
    GROUPED_GROUPS = [
        [
            ["^agent=bp-.*-activation=relu-layers=5.*-width=100$", "^agent=cbp-.*-activation=relu-layers=5.*-width=100$", "^agent=l2-.*-activation=relu-layers=5.*-width=100$", "111111", "100000", "011111", "000000"], # 5
        ],
    ]
    ...
    WITHIN_PLOT_NAMES = [
        [
            f"Layerwise {YLABEL} Distribution",
        ],
    ]
    ...
    if __name__ == '__main__':
        ...
        for i, fig in enumerate(figs):
            ...
            for j, group_set in enumerate(GROUPED_GROUPS[i]):
                current_ax = axs[j]
                groups_to_plot_on_this_ax = defaultdict(lambda : defaultdict(default_list))
                ...
                if not groups_to_plot_on_this_ax:
                    current_ax.set_title(WITHIN_PLOT_NAMES[i][j], fontsize=14, fontweight='bold')
                ...
                for layer, layers_map in groups_to_plot_on_this_ax.items():
                    for group_ind, (group_name_to_plot) in enumerate(group_names):
                        ...
                        x_pos = (total_layers + 2) * group_ind + layer  # Added +2 for spacing between groups of bars
                        ...
                        current_ax.bar(x_pos, mean_over_runs * MULT, yerr=std_over_runs, capsize=5,
                                       color=bar_color, label=label_to_use, ecolor='black', width=0.82, edgecolor='black', linewidth=2)
                try:
                ...
                current_ax.set_title(WITHIN_PLOT_NAMES[i][j], fontsize=19, fontweight='bold')                
                num_x_ticks = 5 
                ...
                if SCILIMITS is not None:
                    current_ax.ticklabel_format(style='scientific', axis='y', scilimits=SCILIMITS) 
                current_ax.yaxis.get_offset_text().set_fontsize(18)
                current_ax.grid(True, linestyle='--', alpha=0.6, axis='y')  # Add grid only for y-axis
            fig.tight_layout()
