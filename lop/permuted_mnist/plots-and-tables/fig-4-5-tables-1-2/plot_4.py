...
GROUPED_GROUPS = [
    ...
    ## Changing the depth
    [
        ["^agent=bp-.*-activation=relu-layers=5.*-width=100$", "^agent=cbp-.*-activation=relu-layers=5.*-width=100$", "^agent=l2-.*-activation=relu-layers=5.*-width=100$", "111111", "100000", "010000", "001000", "000100", "000010", "000000"], # 5
    ],
]
PLOT_NAMES = [
...
def PROCESS_NAME(x):
    ...
    rest_parts = [s.capitalize() for s in processed_parts[2:]]
    ...
    s_part = [second_part]
    if "l2" in first_part.lower():
        s_part = []
    return first_part + ". " + ", ".join(s_part + rest_parts), index
...
if __name__ == '__main__':
    ...
    # figs will have length 1, so not a problem that it is a list. This is because
    # this script is more general
    figs = []
    axss = []
    for i, group_set in enumerate(GROUPED_GROUPS):
        fig, axs = plt.subplots(1, len(GROUPED_GROUPS[i]), figsize=(9, 6))
        if len(GROUPED_GROUPS[i]) == 1:
            axs = [axs]
        figs.append(fig)
        axss.append(axs)

    # As the very first step, fetch all data from all groups (from the pickle file)
    ...
    for i, fig in enumerate(figs):
        ...
        for j, group_set in enumerate(GROUPED_GROUPS[i]):
            ...
            for group_name_to_plot, (data_array_to_plot) in order(groups_to_plot_on_this_ax.items()):
                ...
                current_ax.plot(x_coords, mean_values * MULTIPLY_VALUES, label=f'{PROCESS_NAME(group_name_to_plot)[0]}', color=pallete(color_ind), linewidth=2.5)
            ...
            current_ax.legend(loc='lower left', fontsize=15)
            current_ax.set_xlabel(XLABEL, fontsize=20)
            current_ax.set_ylabel(YLABEL, fontsize=20)
        ...
        fig_name = f"all_layers_separately.pdf"
